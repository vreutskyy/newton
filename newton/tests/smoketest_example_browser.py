...
This script requires the GL viewer and must be run manually. The filename is
intentionally prefixed ``smoketest_`` (not ``test_``) so the unittest runner
does not auto-discover it. Programmatic regression tests for the example
browser live in :mod:`newton.tests.test_example_browser`.
...
Usage:
    uv run python newton/tests/smoketest_example_browser.py                     # all examples, 1 frame
    uv run python newton/tests/smoketest_example_browser.py "mpm_*" --frames 10 # mpm examples, 10 frames
    uv run python newton/tests/smoketest_example_browser.py "robot_h1" "cloth_*" --frames 5
"""
