...
- Fix `SolverVBD(tendon_material_direct=True)` allocating cable material on the opening poses of a step: iteration 0 is the raw inertial predictor, in which every penalty-restrained joint is violated, so a light body carrying routing links moves the cable route by tens of millimetres before the iterations pull it back. The direct solve is exact for the geometry it is given, so it published rest lengths for that discarded pose and left a residue that grew to kilonewton-scale tensions. The transfer now runs only on iterations whose route geometry has stopped moving, controlled by the new `tendon_material_direct_settle_tol` argument (default 1e-2 of a tendon's active length); the accepted end-of-step pose always solves. Material sweeps are unaffected.
