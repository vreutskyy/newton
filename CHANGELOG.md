...
- Fix a dynamic tendon roller whose activation substep already ends past its chord losing `2 * radius * |theta|` of rest length when it deactivates on the next substep: the activation split now books the oriented wrap arc, the same signed quantity the rolling transfer and the deactivation merge use, so the route conserves cable material through a one-substep negative wrap of any size.
- Fix `tendon_total_cable` reporting `2 * radius * |theta|` more cable than the route conserves for a rolling link that starts at a negative oriented wrap: book the oriented arc and warn once at construction, while preserving the positive arc of a fixed half-turn for either winding.
- Fix rolling tendon routes changing total cable material as their wrap angle changes.
- Fix `SolverVBD(tendon_material_direct=True)` evaluating a dynamic tendon roller that changed state at the start of a step against the previous route's rest lengths during the opening `tendon_material_direct_settle_iterations`: the merged span of a deactivated roller (or the new span of an activated one) carried a phantom stretch of half a span for those iterations, which the tendon row turned into kilonewton tensions and a kinetic-energy injection at every route change. The hold now re-bases the rest lengths on the step-start route (merge and split) before the first force evaluation and still transfers no material on those poses.
- Fix `SolverVBD(tendon_material_direct=True)` allocating cable material on the opening poses of a step: iteration 0 is the raw inertial predictor, in which every penalty-restrained joint is violated, so a light body carrying routing links moves the cable route by tens of millimetres before the iterations pull it back. The direct solve is exact for the geometry it is given, so it published rest lengths for that discarded pose and left a residue that grew to kilonewton-scale tensions. Those opening iterations now keep the last solved pose's rest lengths, for the new `tendon_material_direct_settle_iterations` argument's count of iterations (default 4, capped at `iterations - 1`); the accepted end-of-step pose always solves. Material sweeps are unaffected.
- Fix routed tendon attachments flipping between the two intersections of overlapping rolling wrap circles, where no common tangent exists: the previously accepted tangent points are now held there, so pose noise no longer steps the wrap angles, collapses the free span, or spikes the cable tension.
- Fix the overlapping-wrap-circle hold in `update_tendon_attachments` so it now applies only when the required tangent is missing: opposite-winding pairs at d ≤ r_l + r_r, same-winding pairs at d ≤ |r_l − r_r|; same-winding rollers that touch keep their valid external tangent (reported by V. Reutskyy).
- Fix dynamic tendon routing switching a candidate roller on an undefined bypass span: when the two neighboring wrap circles overlap, when the tangent construction between them does not settle, or when the resulting span is shorter than `tendon_route_min_span_ratio` candidate radii, the previous activation state is now held instead of being decided from a collapsed or noise-dominated span. An active candidate that the short span no longer reaches is still released, so a roller the cable has left cannot be held with a negative wrap angle. This removes the rest-length steps, tension spikes, and copy-to-copy divergence such a switch caused.
- Fix the CUDA cooperative direct tendon material kernel latching rejections taken on poses `SolverVBD` discards, which froze a tendon's material state for the rest of the simulation; it now latches only on the accepted end-of-step pose, matching the CPU path. `check_tendon_material()` also reports the offending span's index, free length, compliance, demanded tension and rest-length bound.
